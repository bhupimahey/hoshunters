...
die();
   ...
   function rooms_query($s_search_parametrs,$length,$start){
	...
	if($country)
      $searchset_count .=" LOWER(pfs.country) LIKE '%".$country."%' OR " ;
     ...
     elseif($searchsuburb)
        $searchset_count .=" LOWER(pfs.state) LIKE '%".$searchsuburb."%' OR " ;
      ...
      elseif($searchsuburb)
        $searchset_count .=" LOWER(pfs.city) LIKE '%".$searchsuburb."%' OR " ;
      ...
      elseif($searchsuburb)
        $searchset_count .=" LOWER(pfs.street) LIKE '%".$searchsuburb."%' OR " ;
  ...
  function ajax_listings_list($user_id){			
		...
		$config = array();
        $config["base_url"] = customer_path() . "listings";
        $config["total_rows"] = $iTotalRecords;
        $config["per_page"] = $length;
  ...
  function update_home_request_info(){
	   ...
	   if($_POST){
			  ...
			  $user_infomartion = $this->common_model->get_user_info($user_id);
			  if($user_infomartion)
			  $mobile_verified =$user_infomartion->mobile_verified;
			  else
			  $mobile_verified =0;
			  ...
			  if($mobile_verified==1){
			    $update_data =array("listing_confirmed"=>1);
	            $this->db->where("profile_id",$profile_id);				   
	            $this->db->update("users_profile",$update_data);   
			  }
 ...
 function getUsersFlatmatesPreferences($profile_id) {
  ...
  function confirm_listing() {
	   ...
	   if($query->num_rows() >0 ){
	     $this->db->where('profile_id',$profile_id);  
	     $this->db->update('users_profile',array('listing_confirmed'=>1));
	     
	     $user_id = $this->session->userdata('s_user_id');
	     $this->db->where('account_id',$user_id);  
	     $this->db->update('users',array('mobile_no'=>$mobile_no));
	        ...
	        if($step_type=='roomavailability'){
                ...
                $rooms_types = $this->input->post('room_types_values');
	             ...
	             for($i=1;$i<=$total_room;$i++){
	                 ...
	                 $data = array('class' =>'form-control dateavailable','type'=>'text','name'=> 'date_available['.$i.']', 'id'=> 'date_available'.$i.'',"required"=>"true","value"=>'');
